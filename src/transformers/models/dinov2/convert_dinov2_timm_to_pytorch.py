...
"""Convert DINOv2 and non-distilled DeiT checkpoints from the timm library."""
...
@torch.no_grad()
def convert_dinov2_checkpoint(dinov2_name, pytorch_dump_folder_path):
    """
    ...
    if dinov2_name[-5:] == "in21k":
        base_model = True
        config.patch_size = int(dinov2_name[-12:-10])
        config.image_size = int(dinov2_name[-9:-6])
    else:
        ...
        config.patch_size = int(dinov2_name[-6:-4])
        config.image_size = int(dinov2_name[-3:])
    # size of the architecture
    if "deit" in dinov2_name:
        if dinov2_name[9:].startswith("tiny"):
            config.hidden_size = 192
            config.intermediate_size = 768
            config.num_hidden_layers = 12
            config.num_attention_heads = 3
        elif dinov2_name[9:].startswith("small"):
        ...
        if dinov2_name[4:].startswith("small"):
            config.hidden_size = 768
            config.intermediate_size = 2304
            config.num_hidden_layers = 8
            config.num_attention_heads = 8
        elif dinov2_name[4:].startswith("base"):
            pass
        elif dinov2_name[4:].startswith("large"):
            config.hidden_size = 1024
            config.intermediate_size = 4096
            config.num_hidden_layers = 24
            config.num_attention_heads = 16
        elif dinov2_name[4:].startswith("huge"):
    ...
    timm_model = timm.create_model(dinov2_name, pretrained=True)
    timm_model.eval()
    ...
    print(f"Saving model {dinov2_name} to {pytorch_dump_folder_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dinov2_name",
        default="dinov2_base_patch16_224",
        type=str,
        help="Name of the Dinov2 timm model you'd like to convert.",
    )
    parser.add_argument(
    ...
    convert_dinov2_checkpoint(args.dinov2_name, args.pytorch_dump_folder_path)
